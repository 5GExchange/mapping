#!/usr/bin/python -u
# Copyright 2017 Matthias Rost, Balazs Nemeth
#
...
class Graph(object):
  ...
  def add_edge (self, id, tail, tail_port, head_port, head, bidirected=False,
                **kwargs):
    if (not tail in self.nodes) or (not head in self.nodes):
      raise Exception(
        "Either node {} or node {} was not found while adding edge; "
        "current node set is {}".format(tail, head, self.nodes))

    self._add_edge_one_direction(id, tail=tail, tail_port=tail_port,
                                 head_port=head_port, head=head, **kwargs)
    if bidirected:
      self._add_edge_one_direction(id + "_back", tail=head, tail_port=head_port,
                                   head_port=tail_port, head=tail, **kwargs)

  def _add_edge_one_direction (self, id, tail, tail_port, head_port, head,
                               **kwargs):
  ...
  def initialize_shortest_paths_costs (self):
    ...
    for (u, u_p, v_p, v) in self.edges:
      if self.shortest_paths_costs[u][v] is None:
        self.shortest_paths_costs[u][v] = self.edge[(u, u_p, v_p, v)][
          self._shortest_paths_attribute_identifier]
      elif self.shortest_paths_costs[u][v] > self.edge[(u, u_p, v_p, v)][
        self._shortest_paths_attribute_identifier]:
        self.shortest_paths_costs[u][v] = self.edge[(u, u_p, v_p, v)][
          self._shortest_paths_attribute_identifier]
    ...
    for k in self.nodes:
      for u in self.nodes:
        for v in self.nodes:
          if self.shortest_paths_costs[u][k] is not None and \
                self.shortest_paths_costs[k][v] is not None:
            cost_via_k = self.shortest_paths_costs[u][k] + \
                         self.shortest_paths_costs[k][v]
            if self.shortest_paths_costs[u][v] is None or cost_via_k < \
               self.shortest_paths_costs[u][v]:
...
class Substrate(Graph):
  """ representing the physical network,
      ...
      - bandwidth, cpu, memory, storage, delay are directly taken from the
      respective SG nodes
      node attributes are accessed e.g. via self.node[snode]['cpu'] where
      snode denotes the id of a substrate node

      edges are 4 tuples (tail, tail_p, head_p, head), where tail and head
      denote the respective nodes and
      tail_p and head_p denote the respective ports of the respective nodes.
      edges have the following attributes.
      - delay, bandwidth
      these attributes are accessed e.g. via self.edge[sedge]['cpu'],
      where sedge denotes a 4-tuple
  """
  ...
  def add_node (self, id, type, types, delay, bandwidth, cpu, memory, storage,
                cost=1):
  ...
  def add_edge (self, id, tail, tail_port, head_port, head, delay, bandwidth,
                cost=1):
...
class Request(Graph):
  ...
  nodes have the following attributes
  - id of the node
  - type: the corresponding NF-type or the static string 'SAP', if the node
  refers to a SAP
  - cpu, memory, storage denote the standard requirements for the respective NF
  - allowed_nodes may be set to restrict the potential mapping locations;
  e.g. given a SAP node, the only allowed_node will be the respective SAP

  edges are 4 tuples (tail, tail_p, head_p, head), where tail and head denote
  the respective (virtual) nodes and
  tail_p and head_p denote the respective ports of the respective nodes.
  edges have the following attributes.
  - bandwidth
  ...
  def add_edge (self, id, tail, tail_port, head_port, head, bandwidth):
    if (tail in self.nodes and head in self.nodes):
      ...
      raise Exception(
        "Either the tail ({}) or the head ({}) are not contained in the node "
        "set {}.".format(
          tail, head, self.nodes))
  ...
  def add_delay_requirement (self, id, path, delay):
    ...
    if (set(path) <= set(self.edge_id_to_edge.keys())):
      ...
      raise Exception(
        "Path contains edges which are NOT in request edges: {} vs. {}".format(
          path, self.edge_id_to_edge.keys()))
...
def convert_req_to_request (req):
  ...
  for infra in req.infras:
    ...
    if infra.type != NFFG.TYPE_INFRA:
      raise Exception(
        "infra node is no infra node: {} has type {}".format(infra.id,
                                                             infra.type))
    raise Exception(
      "request cannot contain infrastructure nodes: ".format(infra.id))

  # saps
  for sap in req.saps:
    print "\t adding SAP node {} WITHOUT CONSIDERING CPU, MEMORY or " \
          "STORAGE".format(
      sap.id)
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(sap).items()) + "]"

    if (hasattr(sap, 'delay') and sap.delay is not None and sap.delay > 0) or \
       (hasattr(sap,
                'bandwidth') and sap.bandwidth is not None and sap.bandwidth
           > 0):
      raise Exception(
        "Cannot handle SAP delay ({}) or bandwidth ({}).".format(sap.delay,
                                                                 sap.bandwidth))

    result.add_node(id=sap.id, ntype="SAP", cpu=0, memory=0, storage=0,
                    allowed_snodes=[sap.name])
  ...
  for nf in req.nfs:
    ...
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(nf).items()) + "]"

    # check that bandwidth and delay are not used
    if nf.resources.delay is not None and nf.resources.delay > 0:
      raise Exception(
        "Cannot handle NF delay requirements of NF {}".format(nf.id))

    if nf.resources.bandwidth is not None and nf.resources.bandwidth > 0:
      raise Exception(
        "Cannot handle NF bandwidth requirements of NF {}".format(nf.id))

    result.add_node(id=nf.id, ntype=nf.functional_type, cpu=nf.resources.cpu,
                    memory=nf.resources.mem, storage=nf.resources.storage,
                    allowed_snodes=None)
  ...
  for link in req.links:
    # Matthias: this is just to check that I got the concept right
    if link.type != NFFG.TYPE_LINK_DYNAMIC and link.type != \
       NFFG.TYPE_LINK_STATIC:
      raise Exception("link is neither dynamic nor static: {}".format(link.id))
    raise Exception(
      "Request may not contain static or dynamic links: {}".format(link.id))
  ...
  for sg_link in req.sg_hops:
    ...
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(sg_link).items()) + "]"

    bw_req = sg_link.bandwidth
    if bw_req is None:
      bw_req = 0

    result.add_edge(id=sg_link.id, tail=sg_link.src.node.id,
                    tail_port=sg_link.src.id, head_port=sg_link.dst.id,
                    head=sg_link.dst.node.id, bandwidth=bw_req)

    if sg_link.delay is not None and sg_link.delay > 0:
      # add new novel constraint
      result.add_delay_requirement(id="sg_link_req_{}".format(sg_link.id),
                                   path=[sg_link.id], delay=sg_link.delay)
  ...
  for path_req in req.reqs:
    ...
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(path_req).items()) + "]"

    result.add_delay_requirement(id=path_req.id, path=path_req.sg_path,
                                 delay=path_req.delay)

    if path_req.bandwidth is not None and path_req.bandwidth > 0:
      print "\t\t there exists an additional bandwidth requirement of {} " \
            "units. Augmenting every contained link with the required " \
            "bandwidth.".format(
        path_req.bandwidth)
      ...
      for edge_id in path_req.sg_path:
        ...
        print "\t\t\t increasing bandwidth along edge {} (i.e. {}) from {} to " \
              "{}".format(
          edge_id, edge, bw_req_before, result.edge[edge]['bandwidth'])
...
def convert_nffg_to_substrate (nffg):
  ...
  for infra in nffg.infras:
    ...
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(infra).items()) + "]"
    ...
    # (Balazs): These are not needed, the MILP gets always the available
    # resources
    # # taking into account allocations..
    # for vnf in nffg.running_nfs(infra.id):
    #     print "\t\t\t reducing resources of node {} according to resources
    # {} of VNF {}".format(infra.id, vnf.resources, vnf.id)
    #     result.reduce_available_resources_at_node(infra.id, vnf.resources)
    #
    # print "\t\t final resources of node {} are {}".format(infra.id,
    # result.node[infra.id])
    #
  ...
  for sap in nffg.saps:
    print "\t adding SAP node {} WITHOUT CONSIDERING CPU, MEMORY, BANDWIDTH " \
          "OR DELAY".format(
      sap.id)
    print "\t\t [" + ', '.join(
      "%s: %s" % item for item in vars(sap).items()) + "]"

    # if (sap.delay is not None and sap.delay > 0) or (sap.bandwidth is
    # not None and sap.bandwidth > 0):
    #    raise Exception("Cannot handle SAP delay ({}) or bandwidth ({
    # }).".format(sap.delay, sap.bandwidth))

    if hasattr(sap, 'delay') and sap.delay is not None and sap.delay > 0:
      print "\t\t ignoring {} delay at SAP {} as this is not of importance " \
            "here; setting delay to 0 ".format(
        sap.delay, sap.id)
    if hasattr(sap, 'bandwidth'):
      print "\t\t ignoring {} bandwidth at SAP {} as this is not of " \
            "importance here; setting bandwidth to inf".format(
        sap.bandwidth, sap.id)
  ...
  for edge_link in nffg.links:
    ...
    if edge_link.type == "STATIC":
      ...
      print "\t\t [" + ', '.join(
        "%s: %s" % item for item in vars(edge_link).items()) + "]"
...
class Scenario(object):
  ...
  def compute_allowed_nodes (self, request, vnode):
    substrate_nodes = self.substrate.get_nodes_supporting_type(
      request.get_type_of_node(vnode))
    ...
    if allowed_nodes_raw is None:
      ...
      if set(allowed_nodes_raw) <= set(substrate_nodes):
        ...
        raise Exception(
          "Couldn't resolve allowed nodes for node {} of request".format(vnode,
                                                                         request.id))

        # TODO add some options for realizing different objectives


def construct_name (name, req_id=None, vnode=None, snode=None, vedge=None,
                    sedge=None):
...
class ModelCreator(object):
  ...
  def __init__ (self, scenario, migration_cost_handler=None):
    ...
    # as vedges may be embedded on a single node, we use the following
    # dict to store the respective potential node embeddings
    # self.potential_vedge_embeddings_on_nodes = {}   #a dict of req -->
    # vedge --> [snodes]
  ...
  def run_milp (self):
    ...
    if solutionCount > 0:
      ...
      # interestingly, MIPGap and ObjBound cannot be accessed when there are
      # no variables and the MIP is infeasible..
  ...
  def preprocess (self):
    for req in self.requests:
      ...
      for vnode in req.nodes:
        self.allowed_nodes_copy[req][
          vnode] = self.scenario.compute_allowed_nodes(req, vnode)
        ...
        #             if snode in self.allowed_nodes_copy[req][vtail] and
        # snode in self.allowed_nodes_copy[req][vhead]:
        #                 self.potential_vedge_embeddings_on_nodes[req][
        # vedge].append(snode)
  ...
  def create_variables (self):
    ...
    for req in self.requests:
      ...
      self.var_embedding_decision[req] = self.model.addVar(lb=0.0, ub=1.0,
                                                           obj=0.0,
                                                           vtype=GRB.BINARY,
                                                           name=variable_id)
    ...
    for req in self.requests:
      ...
      for vnode in req.nodes:
        ...
        for snode in allowed_nodes:
          variable_id = construct_name("node_mapping", req_id=req.id,
                                       vnode=vnode, snode=snode)
          self.var_node_mapping[req][vnode][snode] = self.model.addVar(lb=0.0,
                                                                       ub=1.0,
                                                                       obj=0.0,
                                                                       vtype=GRB.BINARY,
                                                                       name=variable_id)
    ...
    for req in self.requests:
      ...
      # TODO the above assumes that arbitrary paths are possible.
      # However, this is not the case as delay constraints might be
      # enforced
      ...
      for vedge in req.edges:
        ...
        for sedge in self.substrate.edges:
          variable_id = construct_name("edge_mapping", req_id=req.id,
                                       vedge=vedge, sedge=sedge)
          self.var_edge_mapping[req][vedge][sedge] = self.model.addVar(lb=0.0,
                                                                       ub=1.0,
                                                                       obj=0.0,
                                                                       vtype=GRB.BINARY,
                                                                       name=variable_id)
    ...
    for snode in self.substrate.nodes:
      ...
      for resource in node_resources:
        ...
        self.var_node_load[snode][resource] = self.model.addVar(lb=0.0,
                                                                ub=GRB.INFINITY,
                                                                obj=0.0,
    ...
    for sedge in self.substrate.edges:
      ...
      self.var_edge_load[sedge] = self.model.addVar(lb=0.0, ub=GRB.INFINITY,
                                                    obj=0.0,
                                                    vtype=GRB.CONTINUOUS,
                                                    name=variable_id)
    ...
    for req in self.requests:
      ...
      for id, _ in req.graph["path_requirements"].keys():
        variable_id = construct_name("path_delay", req_id=req.id) + "_" + str(
          id)
        self.var_path_delay[req][id] = self.model.addVar(lb=0.0,
                                                         ub=GRB.INFINITY,
                                                         obj=0.0,
                                                         vtype=GRB.CONTINUOUS,
                                                         name=variable_id)
    ...
    if self.do_node_load_balancing:
      self.var_node_util_lower_bound = self.model.addVar(lb=0.0, ub=1.0, obj=0.0,
        ...
        #
        #         for snode in self.potential_vedge_embeddings_on_nodes[req][
        # vedge]:
        #             variable_id = construct_name("vedge_mapping_on_snode",
        # req_id=req.id, vedge=vedge, snode=snode)
        #             self.var_vedge_to_snode_mapping[req][vedge][sedge] =
        # self.model.addVar(lb=0.0, ub=1.0, obj=0.0, vtype=GRB.BINARY,
        # name=variable_id)
  ...
  def create_constraints (self):
    ...
    # self.create_constraint_set_vedge_via_snode_variables() #TODO currently
    # not necessary
    ...
    for req in self.requests:
      for vnode in req.nodes:
        ...
        constr_name = construct_name(
          "request_embedding_triggers_node_embeddings", req_id=req.id,
          vnode=vnode)
        ...
        for snode in self.substrate.nodes:
          ...
          expr = LinExpr(
            [(-1.0, self.var_edge_mapping[req][vedge][sedge]) for sedge in
             self.substrate.in_neighbors[snode]]
            +
            [(+1.0, self.var_edge_mapping[req][vedge][sedge]) for sedge in
          ...
          constr_name = construct_name("induce_and_preserve_unit_flows",
                                       req_id=req.id, vedge=vedge, snode=snode)
  ...
  # vedge]:
  #
  #                 expr = LinExpr([(+1.0, self.var_vedge_to_snode_mapping[
  # req][vedge][snode]),
  #                                 (-1.0, self.var_node_mapping[req][vtail][
  # snode])])
  #
  #                 constr_name = construct_name("set_vedge_via_snode_1",
  # req_id=req.id, vedge=vedge, snode=snode)
  #
  #                 self.model.addConstr(expr, GRB.LESS_EQUAL, 0.0,
  # name=constr_name)
  #
  #
  #                 expr = LinExpr([(+1.0, self.var_vedge_to_snode_mapping[
  # req][vedge][snode]),
  #                                 (-1.0, self.var_node_mapping[req][vhead][
  # snode])])
  #
  #                 constr_name = construct_name("set_vedge_via_snode_2",
  # req_id=req.id, vedge=vedge, snode=snode)
  #
  #                 self.model.addConstr(expr, GRB.LESS_EQUAL, 0.0,
  # name=constr_name)
  ...
  # snode])])
  #
  #                 constr_name = construct_name("set_vedge_via_snode_3",
  # req_id=req.id, vedge=vedge, snode=snode)
  #
  #                 self.model.addConstr(expr, GRB.GREATE_EQUAL, -1.0,
  # name=constr_name)
  ...
  def create_constraint_node_loads_standard (self):
    ...
    for node_property in node_properties:
      for snode in self.substrate.nodes:
        expr = LinExpr([(req.node[vnode][node_property],
                         self.var_node_mapping[req][vnode][snode])
                        for req in self.requests for vnode in req.nodes if
                        snode in self.allowed_nodes_copy[req][vnode]])

        expr.addTerms(-1.0, self.var_node_load[snode][node_property])

        constr_name = construct_name("set_node_load_var_standard",
                                     snode=snode) + "_{}".format(node_property)

        self.model.addConstr(expr, GRB.EQUAL, 0.0, constr_name)

        expr = LinExpr([(1.0, self.var_node_load[snode][node_property])])

        constr_name = construct_name("bound_node_load_standard",
                                     snode=snode) + "_{}".format(node_property)

        self.model.addConstr(expr, GRB.LESS_EQUAL,
                             self.substrate.node[snode][node_property],
                             constr_name)

  def create_constraint_node_load_bandwidth (self):
    for snode in self.substrate.nodes:
      expr = LinExpr([
                       (req.edge[vedge]['bandwidth'],
                        self.var_edge_mapping[req][vedge][sedge])
                       for req in self.requests
                       for vedge in req.edges
      ...
      self.model.addConstr(expr, GRB.LESS_EQUAL,
                           self.substrate.node[snode]['bandwidth'], constr_name)

  def create_constraint_edge_load_bandwidth (self):
    for sedge in self.substrate.edges:
      expr = LinExpr([(req.edge[vedge]['bandwidth'],
                       self.var_edge_mapping[req][vedge][sedge])
                      for req in self.requests
                      for vedge in req.edges])
      ...
      self.model.addConstr(expr, GRB.LESS_EQUAL,
                           self.substrate.edge[sedge]['bandwidth'], constr_name)
      ...
      for (id,
        ...
        for vedge in tuple_path:
          ...
          sub_expr = LinExpr([
                             ...
                             [
                               (self.substrate.node[snode]['delay'],
                                self.var_node_mapping[req][vtail][snode])
        ...
        constr_name = construct_name("set_delay",
                                     req_id=req.id) + "_id[{}]".format(id)

        self.model.addConstr(expr, GRB.EQUAL, 0.0, constr_name)

        expr = LinExpr(1.0, self.var_path_delay[req][id])

        constr_name = construct_name("bound_delay",
                                     req_id=req.id) + "_id[{}]".format(id)
  ...
  def plugin_objective_maximize_number_of_embedded_requests_and_minimize_costs (
     self):
    ...
    profit_expr = LinExpr(
      [(2.0 * max_edge_cost, self.var_embedding_decision[req]) for req in
       self.requests])
    cost_expr = LinExpr(
  ...
  def _obtain_solution (self):
    if not isFeasibleStatus(self.status):
      raise Exception(
        "Gurobi's solution does seem to be infeasible. Cannot parse the data!")
      ...
      if self.var_embedding_decision[req].X > 0.5:
        ...
        for vedge in req.edges:
          ...
          if start_node == end_node:
            # we do not need to perform any search: the virtualized functions
            #  are directly connected via the same node
            ...
            mapping.map_edge(vedge, list(reversed(
              substrate_edge_path)))
        ...
        for (id, vedge_path) in req.graph['path_requirements']:
          mapping.set_raw_path_delay(id, vedge_path,
                                     self.var_path_delay[req][id].X)
    ...
    for snode in self.substrate.nodes:
      for resource in node_resources:
        self.solution.set_raw_node_load(snode, resource,
                                        self.var_node_load[snode][resource].X)
...
class Mapping(object):
  ...
  def map_node (self, vnode, snode):
    ...
    if vnode not in self.request.nodes:
      raise Exception("NF {} does not belong to the request {}".format(vnode,
                                                                       self.request.id))
    if snode not in self.substrate.nodes:
      raise Exception(
        "Substrate node {} does not belong to the substrate {}".format(snode,
                                                                       self.substrate.id))
    if snode not in self.scenario.compute_allowed_nodes(self.request, vnode):
      raise Exception(
        "Substrate node {} does not match required type of vnode {} of "
        "request {}".format(
          snode, vnode, self.request.id))
  ...
  def map_edge (self, vedge, substrate_edge_path):
    ...
    if vedge not in self.request.edges:
      raise Exception("vedge {} does not belong to the request {}".format(vedge,
                                                                          self.request.id))
    ...
    for index, (stail, stail_p, shead_p, shead, slinkid) in enumerate(
      ...
      if index == 0:
        if vtail not in self.vnode_to_snode:
          raise Exception(
            "Before adding mappings of edges, please add the node mappings: "
            "vnode {}".format(
              vtail))
        if stail != self.vnode_to_snode[vtail]:
          raise Exception(
            "The substrate path {} does not start at the substrate node {} "
            "onto which the tail of the virtual link {} was mapped".format(
              substrate_edge_path, stail, vedge))
        last_snode = shead
        ...
        if index == len(substrate_edge_path) - 1:
          ...
          if shead != self.vnode_to_snode[vhead]:
            raise Exception(
              "The substrate path {} does not end at the substrate node {} "
              "onto which the head of the virtual link {} was mapped".format(
                substrate_edge_path, shead, vedge))
    ...
    if len(substrate_edge_path) > 0:
      ...
      if self.vnode_to_snode[vtail] != self.vnode_to_snode[vhead]:
        raise Exception(
          "Empty edge mapping implies being hosted on the same node!")
  ...
  def set_raw_path_delay (self, id, vedge_path, delay):
    if (id, vedge_path) not in self.request.graph['path_requirements']:
      raise Exception(
        "Cannot set raw path delay for path {} with id {} as it is not part "
    ...
    if (id, vedge_path) not in self.raw_path_delay:
      ...
      raise Exception(
        "Raw path delay of request {} and id {} was already set.".format(
          self.request.id, id))
...
class GurobiStatus(object):
  ...
  # To obtain a more definitive conclusion, set the DualReductions parameter
  # to 0 and reoptimize.
  UNBOUNDED = 5  # Model was proven to be unbounded. Important note: an
  # unbounded status indicates the presence of an unbounded ray that allows
  # the objective to improve without limit. It says nothing about whether the
  #  model has a feasible solution. If you require information on
  # feasibility, you should set the objective to zero and reoptimize.
  CUTOFF = 6  # Optimal objective for model was proven to be worse than the
  # value specified in the Cutoff parameter. No solution information is
  # available.
  ITERATION_LIMIT = 7  # Optimization terminated because the total number of
  # simplex iterations performed exceeded the value specified in the
  ...
  NODE_LIMIT = 8  # Optimization terminated because the total number of
  # branch-and-cut nodes explored exceeded the value specified in the
  # NodeLimit parameter.
  TIME_LIMIT = 9  # Optimization terminated because the time expended
  # exceeded the value specified in the TimeLimit parameter.
  SOLUTION_LIMIT = 10  # Optimization terminated because the number of
  # solutions found reached the value specified in the SolutionLimit parameter.
  ...
  # difficulties.
  SUBOPTIMAL = 13  # Unable to satisfy optimality tolerances; a sub-optimal
  # solution is available.
  IN_PROGRESS = 14  # A non-blocking optimization call was made (by setting
  # the NonBlocking parameter to 1 in a Gurobi Compute Server environment),
  ...
  def isFeasible (self):
    ...
    if not self.integralSolution and feasibleStatus:
      ...
      if result and not feasibleStatus:
        raise Exception(
          "Solutions exist, but the status ({}) indicated an "
          "infeasibility.".format(
            self.status))
      return result
  ...
  def __str__ (self):
    return "solCount: {0}; status: {1}; objValue: {2}; objBound: {3}; objGap: " \
           "{4}; integralSolution: {5}; ".format(
      self.solCount, self.status, self.objValue, self.objBound, self.objGap,
      self.integralSolution)
...
class ScenarioSolution(object):
  ...
  def set_raw_node_load (self, snode, resource, value):
    ...
    if resource in self.raw_node_load[snode]:
      raise Exception(
        "Raw node load for resource {} on node {} was already set.".format(
          resource, snode))
  ...
  def validate_solution (self, debug_output=True):
    ...
    for resource in basic_node_resources:
      ...
      for snode in self.substrate.nodes:
        ...
        if self.substrate.node[snode][resource] < used_resources:
          raise Exception(
            "The capacity of resource {} on substrate node {} is exceeded by "
            "{} many units.".format(
              resource,
              snode,
    ...
    for snode in self.substrate.nodes:
      ...
      for req in self.requests:
        ...
        if mapping.is_embedded:
          ...
          if debug_output:
            print "\t\t vedges being hosted on {} are {}".format(snode,
                                                                 mapping.get_hosted_vedges_using_snode(
                                                                   snode))
      ...
      if used_resources > self.substrate.node[snode]["bandwidth"]:
        raise Exception(
          "The capacity of resource bandwidth on substrate node {} is "
          "exceeded by {} many units.".format(
            snode,
            self.substrate.node[snode]["bandwidth"] - used_resources))
      self.print_solution()

      check_deviation(
        "Comparison of LP resource allocation of resource bandwidth on node {"
        "} and a posteriori computed allocations do not match.".format(
          snode),
        used_resources,
    ...
    for sedge in self.substrate.edges:
      ...
      for req in self.requests:
        ...
        if mapping.is_embedded:
          ...
          if debug_output:
            print "\t\t vedges being hosted on {} are {}".format(sedge,
                                                                 mapping.get_hosted_vedges_on_sedge(
                                                                   sedge))
      ...
      if used_resources > self.substrate.edge[sedge]["bandwidth"]:
        raise Exception(
          "The capacity of resource bandwidth on substrate edge {} is "
          "exceeded by {} many units.".format(
            sedge,
            self.substrate.edge[sedge]["bandwidth"] - used_resources))

      check_deviation(
        "Comparison of LP resource allocation of resource bandwidth on edge {"
        "} and a posteriori computed allocations do not match.".format(
          sedge),
        used_resources,
    ...
    for req in self.requests:
      ...
      if mapping.is_embedded:
        for (id, vedge_path), delay in req.graph[
          'path_requirements'].iteritems():
          cumulative_delay = 0
          ...
          for vedge in vedge_path:
            ...
            for index, (stail, stail_p, shead_p, shead, slid) in enumerate(
              ...
              if index == 0:
                ...
                if debug_output:
                  print "\t\t .. adding delay of {} for using snode {} for " \
                        "embedding {}".format(
                    delay_of_tail,
                    stail,
          ...
          if cumulative_delay > delay:
            raise Exception(
              "Delay bound of requirement {} is exceeded: {} vs. {}.".format(id,
                                                                             cumulative_delay,
                                                                             delay))

          check_deviation(
            "Comparison of delay computed in LP and a posteriori computed "
            "delay does not match for delay requirement {} of request {} [{"
            "}]".format(
              id, req.id, vedge_path),
            cumulative_delay,
