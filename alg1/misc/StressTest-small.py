...
helpmsg = """StressTest.py options are:
   ...
   vnf sharing
                            probability defined by "--vnf_sharing_same_sg"
                            option.
   --vnf_sharing_same_sg=p  The conditional probablilty of sharing a VNF with
   the
   ...
   --shareable_sg_count=i   The number of last 'i' Service Graphs which could
                            be used for sharing VNFs. Default value is
                            unlimited.
   ...
   --poisson         Generate arrival time differneces with lambda=1.0
   Exponential
                     distribution.
   ...
   --map_only_first_batch           After mapping the first batch, the stress 
                                    test exits, even if the mapping was
                                    successful

   --gen_topo_params=k1:v1,k2:v2... Parameters for NFFG topology generator.
"""


def _shareVNFFromEarlierSG (nffg, running_nfs, nfs_this_sc, p):
  sumlen = sum(
    [l * i for l, i in zip([len(running_nfs[n]) for n in running_nfs],
...
def StressTestCore (seed, loops, use_saps_once, vnf_sharing, multiple_scs,
  ...
  if topo_name == "picotopo":
    ...
    sys.path.append(os.path.abspath('../../test/testframework/generator'))
    import generator
    network = generator.networkx_resource_generator('erdos_renyi_graph',
                                                    **kwargs)
  max_test_lvl = 50000
  test_lvl = 1
    ...
    try:
      ...
      while batched_request is not None:
        ...
        if (len(all_saps_ending) < batch_length or \
          ...
          if do_the_dump and network is not None:
            dump_name = "".join(
              [dump_folder, "/test_lvl-", str(test_lvl), ".nffg"])
      ...
      if not me.backtrack_possible:
        ...
        if poisson:
          ...
          log.warn(
            "Peak mapped VNF count is %s in the last run, test level: %s" %
...
def main (argv):
  ...
  for opt, arg in opts:
    ...
    elif opt == "--gen_topo_params":
      for pararg in arg.split(','):
        key, value = pararg.split(':')
  ...
  returned_test_lvl = StressTestCore(seed, loops, use_saps_once, vnf_sharing,
                                     multiple_scs, max_sc_count,
                                     vnf_sharing_same_sg, fullremap,
                                     batch_length, shareable_sg_count,
                                     sliding_share, poisson, topo_name,
                                     bw_factor, res_factor, lat_factor,
                                     bt_limit, bt_br_factor, outputfile,
